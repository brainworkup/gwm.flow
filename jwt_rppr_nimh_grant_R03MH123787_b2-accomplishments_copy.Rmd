...
An animated version of the plot using `gganimate` can be viewed on the PIs
personal website \@
...
(ref:gwas-panels) Imputation Panels.

```{r panels}
panels <- readr::read_csv("panels.csv")
```

Earlier this year (\~April 2021), we began the process of reimputing the COGENT
genome-wide microarray GWAS data to the NHLBI's Trans-Omics for Precision
Medicine (TOPMed) program reference panel. Version r2 of the TOPMed reference
panel, which is the first version available to the scientific community at
large. It includes 97,256 deeply sequenced and diverse reference samples and
308,107,085 genetic variants distributed across the 22 autosomes and the X
chromosome. As can be seen in **Table 1**, TOPMed substantially improves
imputation accuracy as well as breadth/depth of coverage across the genome
compared to existing reference panels from the 1000 Genomes Project and the
Haplotype Reference Consortium.

(ref:fnote) **Table 1.** GWAS reference panels
...
kableExtra::kbl(
  ...
  kableExtra::add_footnote('(ref:fnote)')
```

NHLBI provides the TOPMed Imputation Server, which is a free next-generation
genotype imputation service that runs in a secured cloud computing environment
supported by the [NHLBI BioData Catalyst
program](https://imputation.biodatacatalyst.nhlbi.nih.gov/#!pages/home).
Comparison of the makeup of publicly available GWAS imputation reference panels
provided by Phase 3 of the 1000 Genomes Project (1000G), the Haplotype Reference
Consortium (HRC), and the Trans-Omics for Precision Medicine (TOPMed) program
across the 22 autosomal chromosomes.

We have imputed to the TOPMed reference panel and have completed some of the
initial post-imputation quality control (QC) procedures on approximately 6/32
COGENT cohorts at this point. In comparison to the HRC and 1000 Genome panels.
